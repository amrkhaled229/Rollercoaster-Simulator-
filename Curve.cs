    ...
    public class Curve
    {
        ...
        private void DrawCurvePoints(Graphics g)
        {
            ...
            for (float t = 0.0f; t <= 1.0; t += 0.05f)
            {
                ...
                g.DrawLine(pn, curvePoint.X, curvePoint.Y, curvePoint.X, curvePoint.Y - 20);
            }
            for (float t = 0.0f; t <= 1.0; t += t_inc)
            {
                curvePoint = CalcCurvePointAtTime(t);
                g.FillEllipse(new SolidBrush(cl), curvePoint.X - 2, curvePoint.Y - 2, 5, 5);
                g.FillEllipse(new SolidBrush(cl), curvePoint.X - 2, curvePoint.Y - 2 - 20, 5, 5);
            }
